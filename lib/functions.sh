...
# [ -f "$DOTFILES_DIR/lib/functions.sh" ] && . "$DOTFILES_DIR/lib/functions.sh"
...
#   lsof        _kport, _display_path_entries (port/process inspection)
#   fzf         _kport, _fzf_search, _fzf_history, _fzf_kill
...
# returns the current OS kernel name: Darwin, Linux, etc.
_get_os() {
    uname -s 2>/dev/null
}

# check if the OS is macOS
# returns: true (exit 0) on macOS, false (non-zero) otherwise.
_is_macos() {
    [[ "$(_get_os)" == "Darwin" ]]
}

# check if the OS is Linux
# returns: true (exit 0) on Linux, false (non-zero) otherwise.
_is_linux() {
    [[ "$(_get_os)" == "Linux" ]]
}

# returns the name of the current shell executable (e.g. bash, zsh)
_get_shell() {
    basename "$SHELL" 2>/dev/null
}
...
# returns the current bash version number if we're actually running under bash
_get_bash_version() {
    if ! _is_bash; then
        printf 'Error: not running inside bash (current shell: %s)\n' "$(_get_shell)" >&2
        return 1
    fi
    printf '%s\n' "$BASH_VERSION"
}
...
_switch_to_bash() {
    _switch_to_bash_usage() {
        printf "usage: _switch_to_bash [bash|bash5|zsh] [true|false]\n" >&2
    }

    local target_shell="${1:-bash5}"
    local login="${2:-false}"
    local SHELL_PATH=""
...
_kport() {
  ...
  if [[ ${#ports[@]} -eq 0 ]]; then
    ...
    [[ -n "${ZSH_VERSION:-}" ]] && print -z "_kport ${ports[*]}"
    return 0
  fi

  for port in "${ports[@]}"; do
    if ! [[ "$port" =~ ^[0-9]+$ ]] || [[ "$port" -lt 1 ]] || [[ "$port" -gt 65535 ]]; then
      printf "_kport: '%s' is not a valid port number (1-65535)\n" "$port"
      continue
    fi
...
^u(←line) ←──────────[DELETE]──────────→ ^k(→line)
M-bs(←word)                              M-d(word→)
^h(char←)                                ^d(char→)
^w(←word)

^d(EOF/char→)  ^c(interrupt)  ^r(search)  ^l(clear)  ^_(undo)  ^y(yank)  ^x^e(editor)

⌥ ⌘ O(Option is Meta / ⌥ Option)
EOF
}
...
_deprecated_fzf_kill() {
  local pids
  pids=$(ps aux | fzf -m --header-lines=1 --prompt="Kill process: " | awk '{print $2}' | tr '\n' ' ')
  [[ -n "$pids" ]] && print -z "kill -9 ${pids}"
}

_deprecated_fzf_jobs() {
  local pids
  pids=$(jobs -l 2>/dev/null | fzf -m --prompt="Kill job: " | awk '{for(i=1;i<=NF;i++) if($i~/^[0-9]+$/) {print $i; break}}' | tr '\n' ' ')
  [[ -n "$pids" ]] && print -z "kill -9 ${pids}"
}
