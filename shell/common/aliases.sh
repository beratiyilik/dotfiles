...
# Sourced by .zshrc / .bashrc via $SHELL_ALIASES_PATH (set in exports.sh).
...
# tools/archive — short commands (preserve old muscle memory)
alias bkf='archive backup'    # timestamped backup with checksum verification
alias tgz='archive pack'      # create a tar.gz archive
alias extr='archive extract'  # extract any supported archive format
alias vault='archive vault'   # encrypt / decrypt GPG tar archives
...
alias kp="_fzf_kill_ports"                   # kill processes on port(s)
alias kport="_fzf_kill_ports"                # backward-compat public name
...
# git
# note: git aliases are defined in config/git/.gitconfig
...
alias zsh="_switch_to_bash zsh -l"                 # switch to zsh
alias bash="_switch_to_bash bash5 -l"              # switch to bash5 (default)
alias bash3="_switch_to_bash bash -l"              # switch to bash3
alias tm="tmux"                                    # tmux shortcut
alias tmlocalai="bash \$DOTFILES/config/tmux/.tmux.localai.sh" # local AI dev session
...
if [[ $IS_MACOS -eq 1 ]]; then
  alias brewfile-update="brew bundle dump --file=\$DOTFILES/config/homebrew/Brewfile --force"
