#!/usr/bin/env zsh
# =============================================================================
# backup.zsh — macOS environment backup script
#
# Usage:
#   backup.zsh [OPTIONS]
#
# Creates a timestamped backup directory containing one .txt file per section
# and a combined backup.txt summary.
#
# Sections:
...
OUTPUT_BASE="${DOTFILES_DIR:-$HOME/dotfiles}/backups"
...
Output structure:
  ...
  └── YYYY-MM-DD_HH-MM-SS/
      ...
      ├── network.txt
      ├── ...
      └── backup.txt    (combined summary of all sections)

Examples:
  $(basename "$0")
  $(basename "$0") --brew --node --python
  $(basename "$0") --all --output "$DOTFILES_DIR/backups"
  $(basename "$0") --dry-run
...
# Appends a titled section to the section file and to backup.txt.
write_section() {
...
backup_env() {
	...
	write_section "$file" "DOTFILES DIRECTORY" \
		"$(capture_path "${HOME}/dotfiles" ls -la "${HOME}/dotfiles")"
...
backup_browsers() {
	...
	_dump_chrome_profiles() {
		...
		# discover all profile directories dynamically
		local -a profiles
		profiles=( "${base}"/Default "${base}"/Profile\ *(N) )
...
main() {
	...
	BACKUP_SUMMARY="${BACKUP_DIR}/backup.txt"
