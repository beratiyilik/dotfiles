#!/usr/bin/env zsh
# =============================================================================
# cleanup.zsh — system cleanup script (macOS)
#
# Usage:
#   cleanup.zsh [OPTIONS] [TARGETS]
#
# Targets:
...
Examples:
  $(basename "$0") --ds --swap
  $(basename "$0") -a --yes
  $(basename "$0") --trash --dry-run
  $(basename "$0") --tmp --dry-run
...
clean_trash() {
	...
	if [[ -d "$user_trash" ]]; then
		...
		if [[ -z "$trash_contents" ]]; then
			...
			run_cmd sudo rm -rf "${user_trash}"/*(N) 2>/dev/null || true
	...
	echo "$vol_trash_list" | while IFS= read -r trashdir; do
		...
		if [[ "$DRY_RUN" == true ]]; then
			...
			run_cmd sudo rm -rf "${trashdir}"/*(N) 2>/dev/null || true
		fi
	done
...
clean_logs() {
	...
	# zsh (N) glob qualifier — no error if no match; avoids bash-only compgen -G
	info "Cleaning quarantine events database..."
	local -a qdb_files
	qdb_files=( "${HOME}"/Library/Preferences/com.apple.LaunchServices.QuarantineEventsV*(N) )

	if (( ${#qdb_files[@]} == 0 )); then
		info "No quarantine events database found"
	else
		local qdb="${qdb_files[1]}"
		if [[ "$DRY_RUN" == true ]]; then
