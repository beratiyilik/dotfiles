#!/usr/bin/env bash
# =============================================================================
# update.sh — system-wide package and software update script (macOS)
#
# Description:
#   Updates everything on the machine, in order:
...
#   Each stage is skipped gracefully when its tool is absent.
#
# Usage:
#   update.sh [OPTIONS]
#
...
#
# Dependencies:
#   - softwareupdate, mas, brew (per-stage; each stage skips when missing)
#   - pnpm or npm (nvm-aware), gem, pip (optional language-runtime stages)
#
# Return Codes:
#   0    Success
...
update_macos() {
	...
	if ! has_cmd softwareupdate; then
